...
@test "Test broadcasts" {
    ...
    run qpid-receive -b ecag-fixml-dev1:$ssl --connection-options "{ transport: ssl, sasl_mechanism: EXTERNAL }" -a "broadcast.ABCFR_ABCFRALMMACC1.TradeConfirmationNCM; { node: { type: queue}, assert: never, create: never }" -m 1 --timeout 5
    [ "$status" -eq "0" ]
}
